...
describe('BruteforceSearch', () => {
  beforeAll(async () => {
    ...
    BruteforceSearch = hnswlib.BruteforceSearch;
  });
  ...
  describe('#initIndex', () => {
    beforeAll(() => {
      index = new BruteforceSearch('l2', 3);
    });
  ...
  describe('#addPoint', () => {
    ...
    it('throws an error if more element is added than the maximum number of elements.', () => {
      index.initIndex(1);
      index.addPoint([1, 2, 3], 0);
      expect(() => { index.addPoint([1, 2, 3], 1) }).toThrow("The number of elements exceeds the specified limit");
    });
  });
  ...
  describe('#searchKnn', () => {
    ...
    describe('when metric space is "ip"', () => {
      beforeAll(() => {
        index = new BruteforceSearch('ip', 3);
        index.initIndex(3);
        index.addPoint([1, 2, 3], 0);
        index.addPoint([2, 3, 4], 1);
        index.addPoint([3, 4, 5], 2);
    ...
    describe('when metric space is "cosine"', () => {
      beforeAll(() => {
        index = new BruteforceSearch('cosine', 3);
        index.initIndex(3);
        index.addPoint([1, 2, 3], 0);
        index.addPoint([2, 3, 4], 1);
        index.addPoint([3, 4, 5], 2);
